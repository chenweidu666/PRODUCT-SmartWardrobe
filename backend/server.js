...
app.get('/api/categories', async (req, res) => {
  try {
    const userId = req.query.user_id || 1; // 默认用户ID为1
...
app.get('/api/categories', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const categories = await categoryManagementDB.getCategories(userId);
...
app.post('/api/categories', authenticateToken, async (req, res) => {
  try {
    ...
    const categoryId = await categoryManagementDB.createCategory(userId, {
      name,
      type,
      icon: icon || '🏷️',
      color: color || '#667eea',
...
app.put('/api/categories/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const updated = await categoryManagementDB.updateCategory(id, userId, {
      name,
      type,
      icon: icon || '🏷️',
      color: color || '#667eea',
...
app.delete('/api/categories/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const deleted = await categoryManagementDB.deleteCategory(id, userId);
    ...
    if (error.message === '该分类下还有衣物，无法删除') {
      res.status(400).json({
        success: false,
...
app.get('/api/categories/stats', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const stats = await categoryManagementDB.getCategoryStats(userId);
...
app.get('/api/categories/:id/items', authenticateToken, async (req, res) => {
  try {
    ...
    const items = await categoryManagementDB.getItemsByCategory(userId, id);
